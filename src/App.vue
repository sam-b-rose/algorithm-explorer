...
export default {
  ...
  methods: {
    ...
    handelResize() {
      ...
      const { left, top } = activeTerm.getClientRects()[0];
      this.vocabPos = { x: left, y: top };
    },
    handleScroll(e) {
    ...
    setTerm({ target }) {
      ...
      this.term = target.textContent;
      const { fontSize, lineHeight } = window.getComputedStyle(target);
      this.termFontStyle = { fontSize, lineHeight };
